...
(function () {
	...
	var url = 'http://118.89.225.203/api/forum';
	feedback.files = [];
	...
	feedback.send = function (content) {
		...
		document.getElementById("submit").innerHTML = '发送中...';
		feedback.uploader = plus.uploader.createUpload(url, {
			method: 'POST'
		}, function (upload, status) {
			window.uploading = false;
			document.getElementById("submit").innerHTML = '发送';
			...
			if (status == 200) {
				...
				if (data.success) {
					mui.toast('发帖成功！')
					feedback.clearForm();
					let oldDriver = plus.webview.getWebviewById('oldDriver/index.html');
					mui.fire(oldDriver, 'refresh', window.sectionID);
					mui.back();
				} else {
		...
		feedback.uploader.addData('sectionID', window.sectionID);
		var userInfo = JSON.parse(plus.storage.getItem('userInfo'));
		feedback.uploader.addData('userid', userInfo.id);
		//添加上传文件
	...
	mui('.mui-popover').on('tap', 'li', function (e) {
		document.getElementById("section").innerHTML = this.children[0].innerHTML;
		window.sectionID = this.children[0].id;
