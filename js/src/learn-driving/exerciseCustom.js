...
mui.init()
let mapping = GetQueryString('mapping')
let questionsJsonPath = `../../testDatabase/${GetQueryString('questionsJson')}`
let wrongList = `wrongList-${GetQueryString('mapping')}-${GetQueryString('type')}`
let rightList = `rightList-${GetQueryString('mapping')}-${GetQueryString('type')}`
...
mui.plusReady(function () {
	...
	switch (mapping) {
		case 'wrong':
			mappingData = JSON.parse(storage.getItem(wrongListTotal))
			break
		case 'favorite':
			mappingData = JSON.parse(storage.getItem(favoriteList))
	}
	initQuestionMenu(mappingData)
	setData({
		mapping: mappingData
	})
})
