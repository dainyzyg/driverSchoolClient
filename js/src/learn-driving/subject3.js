...
document.querySelector(".kaogui").addEventListener('tap', e => {
    mui.openWindow({
        url: `../webview.html?title=${encodeURIComponent('科三考规')}&image=${encodeURIComponent('../images/image-page/k3.png')}`
    });
})
document.querySelector(".kaoshimiji").addEventListener('tap', e => {
    mui.openWindow({
        url: `../webview.html?title=${encodeURIComponent('考试秘籍')}&url=${encodeURIComponent('http://mp.weixin.qq.com/mp/homepage?__biz=MzI3NzA3NTYyNA==&hid=3&sn=21748583d7951ed3d2944ad4dbbc1691')}`
    });
})
...
function toPreorderRecord() {
    mui.openWindow({
        url: 'preorderRecord.html?subject=subject3',
        show: {
            autoShow: true, //页面loaded事件发生后自动显示，默认为true
            aniShow: 'pop-in', //页面显示动画，默认为”slide-in-right“；
            duration: 300 //页面动画持续时间，Android平台默认100毫秒，iOS平台默认200毫秒；
...
function toPreorder() {
    mui.openWindow({
        url: 'preorder.html?subject=subject3',
        show: {
            autoShow: true, //页面loaded事件发生后自动显示，默认为true
            aniShow: 'pop-in', //页面显示动画，默认为”slide-in-right“；
            duration: 300 //页面动画持续时间，Android平台默认100毫秒，iOS平台默认200毫秒；
